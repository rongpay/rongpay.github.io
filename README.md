...
1. 创建订单接口
    ...
    4. 请求参数 
        ...
        |name|是|字符串|张三|付款人姓名|
        |email|否|字符串|zhamgsan@gmeil.com|付款人邮箱|
...
3. 余额查询接口
    1. 使用场景：余额查询
    2. 请求方式：POST
    2. 请求地址：网关地址+ /any-pay/open/merchant/account/balance
    2. **请求头：** X-REQUEST-TOKEN:随机字符串
    4. 请求参数
    ...
    6. 响应参数
        ...
        |参数名称|必须|数据类型|示例| 参数说明|
        |  ----  | ------------  |----  |----  |------------  |
        |balance|是|数值|100.00| 余额|
        |frozenAmt|是|数值|100.00| 冻结金额|
        
    5. 示例
    ```
    请求：
    curl --location 'https://网关+/any-pay/open/merchant/account/balance' \
    --header 'Content-Type: application/json' \
